 ...
 */
 function vcat_install_data($data) {
   global $wpdb;
   global $post;
   global $VCAT_GEO_PI_TABLE ;
   ...
   require_once( ABSPATH . 'wp-config.php');
	...
	if (isset($current->post->lat)) {
		$current_data = array('lat'=>$current->post->lat, 'lng'=>$current->post->lng, 'str'=>$current->post->str, 'plz'=>$current->post->plz, 'ort'=>$current->post->ort);
  	}
    
	if (!is_null($data)) {
		$latlng = vcatGetLatLngFromAddress( $data['str']." ".$data['plz']." ".$data['ort']);
	}  
    ...
    if ($current_data)
    {
        ...
        else 
        {
        	$data=array_merge($data, $latlng);
		   	$update = $wpdb->update($VCAT_GEO_PI_TABLE , $data, array('post_id' => ($post->post_type=="revision"&&isset($current->post->lat))?$current->post->post_id:$post->ID));        	
        }
    }
    elseif (!is_null($data))
    {
