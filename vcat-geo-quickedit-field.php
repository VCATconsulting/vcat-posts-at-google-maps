 ...
 * 
 */
class vcat_geo_quickedit_field {
	/**
	 ...
	 */
	 function render_post_columns( $column_name ) {
	    switch ($column_name) {
	    	case 'post_address':
				global $post;   
	 ...
	 * @param $column_name	the name of the current column		
	 */
	function add_quick_edit($column_name) {
	...
	function quick_edit_save( $post_id ) {
		...
		custom_fields_clean( $new_data ); 
		
	    install_data( $post_id, $new_data );
	} 
...
global $vcquick;
$vcquick = new vcat_geo_quickedit_field();
