...
class vcat_geo_costum_field {
	...
	function custom_fields_init()
	{
	    ...
	    {
	        add_meta_box('custom_fields_meta', __('VCAT Geo Data','vcgmapsatposts'),  array(&$this, 'custom_fields_setup'), $type, 'normal', 'high');
	    }
	} 
	...
	function custom_fields_save( $post_id ) {
	    ...
	    $this->custom_fields_clean( $new_data ); 
		
		$result = $vcdb->install_data( $new_data );
	}
	 ...
	 */
	function custom_fields_clean(&$arr)
	{
	    ...
	    if (is_array($arr))
	    {
	        foreach ($arr as $i => $v)
	        {
	            if (is_array($arr[$i]))
	            {
	                $this->custom_fields_clean($arr[$i]);
...
global $vccostum;
$vccostum = new vcat_geo_costum_field();
